...
/**
 * Export hydrated pages to PDF with:
 * - Text blocks as real selectable PDF text
 * - Annotation canvas captured as transparent PNG overlay
 * 
 * This gives the best of both worlds:
 * - Text remains searchable and selectable
 * - Drawings/shapes/signatures appear as high-quality images
 */
export async function exportHydratedWithAnnotations(
  pages: HydratedPage[],
  annotationCanvases: (HTMLCanvasElement | null)[],
  fileName: string
): Promise<void> {
  const pdfDoc = await PDFDocument.create();
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    // 1. Draw text blocks as selectable text
    for (const block of page.blocks) {
      if (block.type === 'text') {
        await drawTextBlock(pdfPage, block as TextBlock, width, height, fonts);
      } else if (block.type === 'image') {
    ...
    // 2. Overlay annotation canvas as transparent PNG
    const annotationCanvas = annotationCanvases[i];
    if (annotationCanvas) {
      try {
        // Get the canvas as PNG with transparency
        const pngDataUrl = annotationCanvas.toDataURL('image/png');
        const pngBytes = await fetch(pngDataUrl).then(res => res.arrayBuffer());

        // Embed and draw the annotation layer
        const annotationImage = await pdfDoc.embedPng(pngBytes);

        // Draw annotation image to fill the page
        ...
        console.warn(`Failed to embed annotations for page ${i}:`, err);
      }
    }
  }
  ...
  console.log('[PDF Export] Download filename:', downloadName);

  // Try multiple download methods for better browser compatibility
  const downloadBlob = (blob: Blob, filename: string) => {
    // Method 1: Modern approach with URL.createObjectURL
    const url = URL.createObjectURL(blob);
    ...
    // Use a mouseEvent for more reliable triggering
  ...
  console.log('[PDF Export] Download triggered for:', downloadName);
}
