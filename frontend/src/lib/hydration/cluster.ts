...
function groupLinesIntoParagraphs(lines: TextLine[], stats: GlobalStats): Paragraph[] {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    // TIGHTER THRESHOLDS: Less aggressive merging for better visual accuracy
    const normalLineGap = stats.dominantLineHeight * 0.25; // Was 0.4
    const paragraphGap = stats.dominantLineHeight * 0.5;   // Was 1.0

    // Check for paragraph break conditions
    const isLargeGap = verticalGap > paragraphGap;
    const isDifferentFontSize = Math.abs(currLine.fontSize - prevLine.fontSize) > 1; // Was 2
    const isSignificantIndentChange = Math.abs(currLine.indent - prevLine.indent) > stats.dominantFontSize * 2; // Was 3
    const isPrevLineShort = (prevLine.maxX - prevLine.minX) < (stats.masterGrid.margins.right - stats.masterGrid.margins.left) * 0.5; // Was 0.6

    // Check if lines are in different X regions (columns)
    const xOverlap = Math.min(currLine.maxX, prevLine.maxX) - Math.max(currLine.minX, prevLine.minX);
    const minWidth = Math.min(currLine.maxX - currLine.minX, prevLine.maxX - prevLine.minX);
    const noHorizontalOverlap = xOverlap < minWidth * 0.2; // Lines don't overlap horizontally

    // Check if current line looks like a header
    const isHeader = currLine.fontSize > stats.typography.body * 1.1;
    const prevWasHeader = prevLine.fontSize > stats.typography.body * 1.1;

    // Check if current line starts with list marker
    const isListItem = /^[•\-\*]\s|^\d+[\.\)]\s|^[a-zA-Z][\.\)]\s/.test(currLine.text);

    // Decide if new paragraph - MORE LIKELY TO BREAK with tighter thresholds
    const shouldBreak =
      isLargeGap ||
      noHorizontalOverlap || // NEW: Different column positions = separate blocks
      isDifferentFontSize ||
      isSignificantIndentChange ||
      (isPrevLineShort && verticalGap > normalLineGap) ||
      isHeader !== prevWasHeader; // Font size changes always break
...
// =============================================================================
// 5. WHITESPACE-BASED COLUMN DETECTION (N-column support)
// =============================================================================

/**
 * Find vertical gaps (empty strips) in the page that indicate column boundaries.
 * Works for any number of columns by analyzing X-position distribution.
 */
function findVerticalGaps(runs: TextRunRaw[], pageDims: { width: number; height: number }, stats: GlobalStats): number[] {
  if (runs.length === 0) return [];

  const pageWidth = pageDims.width;
  const minGapWidth = stats.averageCharWidth * 3; // Gap must be at least 3x char width

  // Create a histogram of X coverage
  // Divide page into buckets and count text coverage
  const bucketCount = 100;
  const bucketWidth = pageWidth / bucketCount;
  const coverage = new Array(bucketCount).fill(0);

  runs.forEach(run => {
    const startBucket = Math.floor(run.x / bucketWidth);
    const endBucket = Math.floor((run.x + run.width) / bucketWidth);
    for (let i = Math.max(0, startBucket); i <= Math.min(bucketCount - 1, endBucket); i++) {
      coverage[i]++;
    }
  });

  // Find gaps - contiguous empty buckets
  ...
  for (let i = 0; i < bucketCount; i++) {
    if (coverage[i] === 0) {
      ...
      if (gapStart !== -1) {
        const gapEnd = i;
        const gapWidthPx = (gapEnd - gapStart) * bucketWidth;
        if (gapWidthPx >= minGapWidth) {
          gaps.push({
            start: gapStart * bucketWidth,
            end: gapEnd * bucketWidth,
            width: gapWidthPx
          });
        }
        gapStart = -1;
  ...
  // Sort gaps by width (largest first) and take significant ones
  gaps.sort((a, b) => b.width - a.width);

  // Return gap midpoints as column boundaries
  const boundaries: number[] = [];
  for (const gap of gaps.slice(0, 5)) { // Max 5 column boundaries (6 columns)
    if (gap.width >= minGapWidth * 2) { // Only significant gaps
...
/**
 * Assign runs to columns based on detected vertical gaps.
 */
function assignRunsToColumns(runs: TextRunRaw[], columnBoundaries: number[]): TextRunRaw[][] {
  const numColumns = columnBoundaries.length + 1;
  const columns: TextRunRaw[][] = Array.from({ length: numColumns }, () => []);
  ...
  for (const run of runs) {
    ...
    for (let i = 0; i < columnBoundaries.length; i++) {
      if (runCenter > columnBoundaries[i]) {
        columnIndex = i + 1;
      }
    }

    columns[columnIndex].push(run);
  }
...
function detectAndSplitColumns(paragraphs: Paragraph[], stats: GlobalStats, pageDims: { width: number; height: number }): Paragraph[][] {
  ...
  // Collect all runs from all paragraphs for gap detection
  const allRuns: TextRunRaw[] = [];
  ...
  // Find vertical gaps
  const columnBoundaries = findVerticalGaps(allRuns, pageDims, stats);

  if (columnBoundaries.length === 0) {
    // Single column layout
    console.log('  → No column gaps found, single column layout');
    return [[...paragraphs].sort((a, b) => a.minY - b.minY)];
  }

  console.log(`  → Found ${columnBoundaries.length} column boundaries at: ${columnBoundaries.map(b => b.toFixed(0)).join(', ')}`);

  // Assign paragraphs to columns based on their center X position
  const numColumns = columnBoundaries.length + 1;
  const columnParas: Paragraph[][] = Array.from({ length: numColumns }, () => []);

  paragraphs.forEach(para => {
    const paraCenterX = (para.minX + para.maxX) / 2;
    let columnIndex = 0;

    for (let i = 0; i < columnBoundaries.length; i++) {
      if (paraCenterX > columnBoundaries[i]) {
        columnIndex = i + 1;
      }
    }

    columnParas[columnIndex].push(para);
  });

  // Sort each column by Y position
  return columnParas.map(col => col.sort((a, b) => a.minY - b.minY));
}
...
function paragraphToTextBlock(
  ...
  const x = (para.minX / pageDims.width) * 100;
  const y = (para.minY / pageDims.height) * 100;
